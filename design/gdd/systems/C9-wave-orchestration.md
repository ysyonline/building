...
| v1.0.1 | 2026-09-22 | **F3 合流勘误批 A3 联动（GW-P2-015，F3 R-3 规范写法，主理人代落）**：§4「C8 消费精化」注记中 `intent-scripts.json` 键位写法精化为 `ai-scripts.json::intentScripts`（F3 v1.0.1 R-3 单文件裁定）。两处措辞零结构改动（§4 注记＋§7 依赖表行） |
