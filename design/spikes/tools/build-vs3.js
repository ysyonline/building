#!/usr/bin/env node
'use strict';
/* build-vs3.js —— 组装 graybox-vs.html（VS-3 战斗核心批次，VS-2 底座原地迭代）
 * 结构：_vs3-shell.html 头（含 UI/样式）+ _vs3-app.js（应用逻辑，F2/F1/F4/C1/C5/C10/C8/RENDER/演示 分区）
 ...
 * 禁止与 build-vs2.js / build-spike.js 混用（各自对应不同源片）。
 */
const fs = require('fs');
const path = require('path');

const DIR = __dirname;
const SHELL = path.join(DIR, '_vs3-shell.html');
const APP = path.join(DIR, '_vs3-app.js');
...
if (app.indexOf('window.VS3Selftest') < 0) { console.error('FAIL: app missing VS3Selftest bridge'); process.exit(1); }
