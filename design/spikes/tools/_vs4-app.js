...
function boot() {
  ...
  function c7Check() {
    ...
    /* 部署：容量满拒绝 + 成功扣费 */
    var cf = C7.exec({ type: 'DEPLOY_UNIT', cellId: '5_0_1', templateId: 'garrison_squad' });   // 垛口容量 1，初始 D_2 占位
    chk('deploy_cell_full', !cf.ok && cf.reason === 'CELL_FULL', JSON.stringify(cf));
    var t1 = C6.treasury();
    ...
    /* 破产=禁止下单：压到余额 < 造价 */
    (function drain() { while (C6.treasury() >= TABLES.economy.cost.build.rollingStock) { C7.exec({ type: 'DEPLOY_UNIT', cellId: '5_1_1', templateId: 'garrison_squad' }); if (!C6.canAfford({ kind: 'BUILD_FACILITY', facilityKind: 'rollingStock' })) break; } })();
    var t4 = C6.treasury();
    var drained = [];
    while (C6.treasury() >= TABLES.economy.cost.deploy.garrisonSquad) drained.push(C7.exec({ type: 'DEPLOY_UNIT', cellId: '5_1_1', templateId: 'garrison_squad' }));
